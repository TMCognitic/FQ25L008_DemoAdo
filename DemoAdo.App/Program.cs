...
using (SqlConnection connection = new SqlConnection(@"Data Source=(localdb)\MSSQLLocalDB;Initial Catalog=DemoAdo;Integrated Security=True;Encrypt=True;Trust Server Certificate=True;"))
{
    connection.Open();
    int result = (int)connection.ExecuteScalar("SELECT COUNT(*) FROM Personne;");
    Console.WriteLine($"Nombre de ligne(s) dans ma table Personne : {result}");
    ...
    DateTime now = (DateTime)connection.ExecuteScalar("SELECT GETDATE();");
    ...
    IEnumerable<Personne> personnes = connection.ExecuteReader("SELECT 'Personne' AS Type, Id, Nom, Prenom, Email, Tel FROM Personne;", dr => new Personne((int)dr["Id"], (string)dr["Nom"], (string)dr["Prenom"], (string)dr["Email"], dr["Tel"] as string));
    ...
    string email = "bidule.doe@test.be";
    ...
    //Console.WriteLine(connection.State);
    int rows = connection.ExecuteNonQuery("[dbo].[CSP_AjoutPersonne]", true, new { Nom = "Test", Prenom = "Test", Email = "test3@test.be" });
    Console.WriteLine($"Nombre de ligne(s) modifiée: {rows}");
}
